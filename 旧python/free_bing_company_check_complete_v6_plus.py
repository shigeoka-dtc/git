...
# ✅ スニペットに会社名が含まれているか？
def is_valid_snippet(company, snippet):
...
def search_bing(driver, company):
    ...
    driver.get(url)
    time.sleep(random.uniform(2.0, 4.0))  # ノートPC向け調整
    results = []
...
def analyze_company(company):
    ...
    try:
        ...
        results_sorted = sorted(
            [
                r for r in results
                if not is_low_quality(r[1], r[2]) and is_valid_snippet(company, r[1])
            ],
